...
def migrate_db():
    ...
    # users: password_hash
    for col in [("password_hash", "TEXT DEFAULT ''"), ("role", "TEXT DEFAULT 'user'"), ("totp_secret", "TEXT DEFAULT ''"), ("totp_enabled", "INTEGER DEFAULT 0")]:
        try:
            db.execute(f"SELECT {col[0]} FROM users LIMIT 0")
        except sqlite3.OperationalError:
            db.execute(f"ALTER TABLE users ADD COLUMN {col[0]} {col[1]}")
    db.commit()
    db.close()
...
def create_user(username: str, password: str, role: str = "user") -> dict:
    ...
    try:
        db.execute(
            ...
            (uid, username, role, pw_hash, time.time())
        )
        db.commit()
    ...
    db.close()
    return {"id": uid, "username": username, "theme": "dark", "role": role, "created": time.time()}

def require_user(request: Request) -> dict:
    """Extract user from X-User-Id header or query param."""
...
def require_admin(request: Request) -> dict:
    """Extract user and verify they are an admin."""
...
async def login(request: Request):
    ...
    if existing:
        ...
        if not pw_hash:
            ...
            db.commit()
            db.close()
        elif not bcrypt.checkpw(password.encode(), pw_hash.encode()):
            return JSONResponse({"error": "Wrong password"}, status_code=401)

        # Check TOTP if enabled
        if existing.get("totp_enabled"):
            if not totp_code:
                # Signal that TOTP is required — don't log in yet
                return JSONResponse({"error": "TOTP code required", "totp_required": True}, status_code=401)
            secret = existing.get("totp_secret", "")
            if not pyotp.TOTP(secret).verify(totp_code, valid_window=1):
                return JSONResponse({"error": "Invalid TOTP code"}, status_code=401)
...
async def change_password(request: Request):
    ...
    if not old_password or not new_password:
        return JSONResponse({"error": "Old and new password required"}, status_code=400)
    if len(new_password) < 4:
        return JSONResponse({"error": "New password must be at least 4 characters"}, status_code=400)

    pw_hash = user.get("password_hash", "")
    if pw_hash and not bcrypt.checkpw(old_password.encode(), pw_hash.encode()):
        return JSONResponse({"error": "Wrong password"}, status_code=401)
...
async def totp_enroll(request: Request):
    """Generate a TOTP secret for the user. Returns secret + QR code SVG."""
    user = require_user(request)
    if user.get("totp_enabled"):
        return JSONResponse({"error": "TOTP already enabled. Disable it first."}, status_code=400)
    ...
    otpauth = pyotp.totp.TOTP(secret).provisioning_uri(
        name=user["username"], issuer_name="Ollama Chat"
    )
...
async def totp_verify(request: Request):
    """Verify a TOTP code and enable TOTP for the user. Call after /enroll."""
    user = require_user(request)
    if user.get("totp_enabled"):
        return JSONResponse({"error": "TOTP already enabled"}, status_code=400)
    body = await request.json()
    code = body.get("code", "")
    secret = user.get("totp_secret", "")
    if not secret:
        return JSONResponse({"error": "Enroll TOTP first"}, status_code=400)
...
async def totp_disable(request: Request):
    """Disable TOTP for the user. Requires password or current TOTP code."""
    user = require_user(request)
    body = await request.json()
    password = body.get("password", "")
    code = body.get("code", "")
    ...
    if password:
        ...
        if not pw_hash or not bcrypt.checkpw(password.encode(), pw_hash.encode()):
            return JSONResponse({"error": "Wrong password"}, status_code=401)
    elif code and user.get("totp_secret"):
        if not pyotp.TOTP(user["totp_secret"]).verify(code, valid_window=1):
            return JSONResponse({"error": "Invalid TOTP code"}, status_code=401)
    else:
        return JSONResponse({"error": "Password or TOTP code required"}, status_code=400)
    db = get_db()
    db.execute("UPDATE users SET totp_enabled=0, totp_secret='' WHERE id=?", (user["id"],))
    db.commit()
    db.close()
...
async def admin_reset_password(request: Request):
    ...
    if not target_id or not new_password:
        return JSONResponse({"error": "user_id and new_password required"}, status_code=400)
    if len(new_password) < 4:
